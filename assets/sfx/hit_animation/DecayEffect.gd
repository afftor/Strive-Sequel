...
func play():
	...
	tw.interpolate_property(get_material(), "shader_param/bordmwidth", 0.1 , 2.0, 1, Tween.TRANS_LINEAR, Tween.EASE_OUT,0.5)
	tw.interpolate_property(get_material(), "shader_param/bordmval",0.5 , 1.0, 1, Tween.TRANS_LINEAR, Tween.EASE_OUT, 0)
	tw.interpolate_property(get_material(), "shader_param/crop", 1.0 , 0.0, 0.5, Tween.TRANS_LINEAR, Tween.EASE_OUT, 1)
	tw.start()
