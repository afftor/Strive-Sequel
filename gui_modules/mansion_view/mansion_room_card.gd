...
func setup(view_node):
	...
	$Body/Columns/LeftColumn/Actions/MoveButton.connect("pressed", self, "on_move")
	$Body/Columns/LeftColumn/Actions/CraftButton.connect("pressed", self, "on_craft")
	$Body/Columns/LeftColumn/Actions/DemolishButton.connect("pressed", self, "on_demolish")
	$Body/Columns/LeftColumn/RepairButton.connect("pressed", self, "on_repair")
	$Body/Columns/LeftColumn/CancelButton.connect("pressed", self, "on_cancel")
	visible = false
...
func fit_to_body():
	var wanted = $Body.get_combined_minimum_size().y + BODY_MARGINS
...
#Sits just right of the slot, flips to its left when it would run off the screen, and hangs
#upwards off it rather than downwards when the slot is low.
...
func rebuild():
	...
	$Body/Columns/LeftColumn/Actions/CraftButton.visible = false
	$Body/Columns/LeftColumn/OrderHeader.visible = false
	$Body/Columns/LeftColumn/OrderList.visible = false
	$Body/Columns/LeftColumn/CancelButton.visible = false
	$Body/Columns/LeftColumn/YieldHeader.visible = false
	$Body/Columns/LeftColumn/YieldList.visible = false
	$Body/Columns/LeftColumn/UpgradeHeader.visible = false
	$Body/Columns/LeftColumn/UpgradeList.visible = false
...
func update_text_sections():
	for path in ['Subtitle', 'Descript', 'Functions']:
		var label = $Body/Columns/LeftColumn.get_node(path)
		label.visible = label.text.strip_edges() != ''
	fit_wrapped_label($Body/Columns/LeftColumn/Descript)
...
func description_fits():
	var label = $Body/Columns/LeftColumn/Descript
	if !label.visible:
		return true
...
func build_for_broken():
	$Body/Columns/LeftColumn/Title.text = tr("MANSIONVIEW_BROKEN")
	$Body/Columns/LeftColumn/Subtitle.text = ""
	$Body/Columns/LeftColumn/Descript.text = tr("MANSIONVIEW_BROKENHINT")
	$Body/Columns/LeftColumn/Functions.text = ""
	$Body/Columns/PeopleColumn/Occupants.visible = false
	$Body/Columns/LeftColumn/Actions.visible = false
	$Body/BuildList.visible = false
	$Body/Columns/LeftColumn/RepairButton.visible = true
	$Body/Columns/LeftColumn/RepairButton.text = tr("MANSIONVIEW_REPAIR")
...
func build_for_scaffolding():
	...
	$Body/Columns/LeftColumn/Title.text = view.build_label(build)
	$Body/Columns/LeftColumn/Subtitle.text = view.build_eta_text(build)
	$Body/Columns/LeftColumn/Descript.text = tr("MANSIONVIEW_BUILDINGHINT")
	$Body/Columns/LeftColumn/Functions.text = "%s %d%%" % [tr("MANSIONVIEW_PROGRESS"),
		int(build.progress * 100.0 / max(1.0, build.limit))]
	$Body/Columns/PeopleColumn/Occupants.visible = false
	$Body/Columns/LeftColumn/Actions.visible = false
	$Body/BuildList.visible = false
	$Body/Columns/LeftColumn/RepairButton.visible = false
	$Body/Columns/LeftColumn/CancelButton.visible = true
	$Body/Columns/LeftColumn/CancelButton.text = tr("MANSIONVIEW_CANCELBUILD")
...
func build_for_empty():
	set_card_width(BUILD_CARD_WIDTH)
	$Body/Columns/LeftColumn/Title.text = tr("MANSIONVIEW_BUILDHERE")
	$Body/Columns/LeftColumn/Subtitle.text = ""
	$Body/Columns/LeftColumn/Descript.text = ""
	$Body/Columns/LeftColumn/Functions.text = ""
	$Body/Columns/PeopleColumn/Occupants.visible = false
...
func setup_build_row(row, code, data, check):
	...
	row.get_node('Bonus').text = slots_text(code)
...
func build_for_room(current):
	$Body/BuildList.visible = false
	$Body/Columns/LeftColumn/RepairButton.visible = false
	$Body/Columns/LeftColumn/Actions.visible = true
	$Body/Columns/LeftColumn/Title.text = tr(RoomTypes.get_name_key(current.type))
	...
	$Body/Columns/LeftColumn/Subtitle.text = ""
	$Body/Columns/LeftColumn/Descript.text = tr(RoomTypes.get_descript_key(current.type))
	$Body/Columns/LeftColumn/Functions.text = upgrades_text(current)
	build_yield_list(current)
	build_craft_button(current)

	$Body/Columns/LeftColumn/Actions/DemolishButton.disabled = !MansionLayout.can_demolish(view.layout(), view.floor_index(), slot_code).ok
	build_move_button()
...
func build_craft_button(current):
	var button = $Body/Columns/LeftColumn/Actions/CraftButton
...
func build_yield_list(current):
	input_handler.ClearContainer($Body/Columns/LeftColumn/YieldList, ['Row'])
	...
	$Body/Columns/LeftColumn/YieldHeader.visible = !rows.empty()
	$Body/Columns/LeftColumn/YieldList.visible = !rows.empty()
	if rows.empty():
		return
	$Body/Columns/LeftColumn/YieldHeader.text = tr("MANSIONVIEW_ROOMYIELDS")
	for entry in rows:
		var row = input_handler.DuplicateContainerTemplate($Body/Columns/LeftColumn/YieldList, 'Row')
...
func build_move_button():
	var button = $Body/Columns/LeftColumn/Actions/MoveButton
...
func build_upgrade_list(current):
	var build = build_data()
	$Body/Columns/LeftColumn/CancelButton.visible = build != null
	if build != null:
		$Body/Columns/LeftColumn/CancelButton.text = tr("MANSIONVIEW_CANCELBUILD")
	input_handler.ClearContainer($Body/Columns/LeftColumn/UpgradeList, ['Row'])
	var codes = RoomTypes.get_type(current.type).upgrades
	$Body/Columns/LeftColumn/UpgradeHeader.visible = !codes.empty()
	$Body/Columns/LeftColumn/UpgradeHeader.text = tr("MANSIONVIEW_ROOMUPGRADES")
	$Body/Columns/LeftColumn/UpgradeList.visible = !codes.empty()
	for code in codes:
		var row = input_handler.DuplicateContainerTemplate($Body/Columns/LeftColumn/UpgradeList, 'Row')
...
func setup_upgrade_row(row, current, code, build):
	...
	if !maxed:
		globals.connecttexttooltip(row, upgrade_hint(code, level, next_level), true,
			view.get_node("Overlay/TextTooltip"), self)
...
func upgrade_descriptions_fit():
	for row in $Body/Columns/LeftColumn/UpgradeList.get_children():
		if !row.visible:
			continue
		var descript = row.get_node('Descript')
		var code = row.get_meta('type')
...
func build_order_list(current):
	...
	$Body/Columns/LeftColumn/OrderList.visible = owned and crafts
	$Body/Columns/LeftColumn/OrderHeader.visible = $Body/Columns/LeftColumn/OrderList.visible
	if !$Body/Columns/LeftColumn/OrderList.visible:
		return
	$Body/Columns/LeftColumn/OrderHeader.text = tr("MANSIONVIEW_ORDERHEADER")
	input_handler.ClearContainer($Body/Columns/LeftColumn/OrderList)
	var offered = 0
	for queue in [job + '_item', job + '_material']:
		for task_id in ResourceScripts.game_res.crafting_lists[queue]:
			var button = input_handler.DuplicateContainerTemplate($Body/Columns/LeftColumn/OrderList)
