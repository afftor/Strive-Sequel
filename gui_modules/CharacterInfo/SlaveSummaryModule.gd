...
func show_summary():
	...
	for i in ['physics','wits','charm','sexuals']:
		get_node("VBoxContainer2/TextureRect3/" + i).text = globals.base_stat_value_text(person, i)
		get_node("VBoxContainer2/TextureRect4/" + i + '2').text = globals.base_stat_cap_text(person, i)
...
var unique_dict = { #shows available talk characters. Scenes go in order from higher priority and reqs to lower. No scenes isn't supported yet
	...
	lira = {
		...
		scenes = [
			...
			{code = 'lira_quest3_start_1', reqs = [{type = 'active_quest_stage', value = 'lira_quest_3', stage = 'start'}]},
			{code = 'lira_quest3_report_lira_1', reqs = [{type = 'active_quest_stage', value = 'lira_quest_3', stage = 'report_lira'}]},
