...
func hp_update(node, args):
	...
	if args.damage_float:
		if crit_display.has(node):
			...
			tween.interpolate_callback(ResourceScripts.core_animations, delay, 'FloatTextArgs', {node = node, text = str(ceil(args.damage)) + '!', type = args.type, size = 500, color = args.color, time = 1, fadetime = 0.5, offset = Vector2(0,0)})
