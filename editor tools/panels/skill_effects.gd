...
func add_rec():
	var tmp = editor_core.skill_eff_template_scene.instance()
	tmp.parent = self
	tmp.newrec = true
	add_child(tmp)
	tmp.popup()
...
func fill_list():
	...
	for c in tres:
		var text
		if typeof(c) == TYPE_STRING: text = c
		elif c.has('description'): text = c.description
		else: text = "templated effect - %s" % c.sub_effects[0]
