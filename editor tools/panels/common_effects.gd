...
func add_rec():
	var tmp = editor_core.common_effect_scene.instance()
	tmp.parent = self
	add_child(tmp)
	tmp.popup()
...
func delete_rec():
	if stat_node.get_selected_items().size() == 0: return
	var pos = stat_node.get_selected_items()[0]
	tres.erase(stat_node.get_item_text(pos))
	update_val()
#	fill_list()
...
func fill_list():
	...
	for c in tres:
		var text
		if c.has('description'): text = c.description
		else: text = editor_core.display_res(c)
