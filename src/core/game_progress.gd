...
func fix_serialization():
	...
	if !seen_events.has("lira_quest_3_intro") && !timed_event_exists("lira_quest_3_intro") && completed_quests.has('lira_quest_2') && !completed_quests.has('lira_quest_3'):
		globals.common_effects([{code = 'progress_quest', value = 'lira_quest_3', stage = 'start'}, {code = 'add_timed_event', value = "lira_quest_3_intro", args = [{type = 'add_to_date', date = [3,3], hour = 1}]}])
