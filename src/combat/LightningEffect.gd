...
#One knob for the whole bolt: every stroke width, the fork reach and the impact
#flash scale with it. The zigzag path itself is untouched, so the bolt still
#runs between the same two points.
...
#Soft-edged strips: Line2D maps the texture's V axis across the stroke width, so
#a strip that fades to transparent at both edges is the closest Line2D gets to a
#blur. Without one the "glow" is just a wider flat band with a hard edge.
#Generated by tools/anim_lab/gen_line_glow.py.
const GLOW_TEX = preload("res://assets/sfx/line_glow.png")
const CORE_TEX = preload("res://assets/sfx/line_core.png")


#profile: 'glow' - wide falloff, 'core' - narrow, null - crisp, no texture
func _make_line(line_width, color, profile = null):
	var line = Line2D.new()
	line.width = line_width * BOLT_SCALE
	line.default_color = color
	#Line2D's own antialiasing is what leaves a perpendicular notch on every
	#vertex; the soft-edged texture does the smoothing instead.
	line.antialiased = false
	#Sharp joints leave a notch on every vertex, and this polyline is nothing but
	#sharp vertices. Round joints and caps are what the web stand gets for free
	#from stroke-linejoin: round.
	line.joint_mode = Line2D.LINE_JOINT_ROUND
	line.begin_cap_mode = Line2D.LINE_CAP_ROUND
	line.end_cap_mode = Line2D.LINE_CAP_ROUND
	line.round_precision = 12
	if profile == 'glow':
		line.texture = GLOW_TEX
		line.texture_mode = Line2D.LINE_TEXTURE_STRETCH
	elif profile == 'core':
		line.texture = CORE_TEX
		line.texture_mode = Line2D.LINE_TEXTURE_STRETCH
...
#A fork drawn as one straight segment of constant width reads as a scratch, not
#as lightning - it was the odd tick sticking out of the bolt. Tapering it to
#nothing at the tip and bending it turns it into a branch.
func _make_fork():
	var line = _make_line(4.0, Color(0.62, 0.88, 1.0, 0.70), 'core')
	var curve = Curve.new()
	curve.add_point(Vector2(0.0, 1.0))
	curve.add_point(Vector2(0.55, 0.55))
...
func _make_charge():
	charge_lines = [
		_make_line(22.0, Color(0.24, 0.68, 1.0, 0.40), 'glow'),
		_make_line(6.0, Color(0.55, 0.90, 1.0, 0.92), 'core'),
		_make_line(2.4, Color(0.88, 0.98, 1.0, 0.95))
	]
...
func _make_rays():
	...
	for index in range(target_nodes.size()):
		...
		#Halo is two stacked layers rather than one: additive blending multiplies
		#colour by alpha, so a single wide line at low alpha lands within a few
		#levels of the background and cannot be seen. Core alpha is held low
		#enough that core plus filament stay under 1.0 and the middle does not
		#clip to flat white - measured at 255,255,255 across 7 px before.
		var wide = _make_line(40.0 if index == 0 else 30.0,
			Color(0.16, 0.46, 1.0, 0.22 if index == 0 else 0.17), 'glow')
		var glow = _make_line(26.0 if index == 0 else 20.0,
			Color(0.20, 0.58, 1.0, 0.34 if index == 0 else 0.26), 'glow')
		var core = _make_line(9.0 if index == 0 else 7.0,
			Color(0.30, 0.78, 1.0, 0.55), 'core')
		var hot = _make_line(2.8 if index == 0 else 2.2,
			Color(0.90, 0.97, 1.0, 0.95))
		var fork_a = _make_fork()
		var fork_b = _make_fork()
		var impact = _make_line(4.0, Color(0.76, 0.95, 1.0, 0.95), 'core')
		for line in [wide, glow, core, hot, fork_a, fork_b, impact]: line.visible = false
		rays.append({
			branch = index > 0,
			delay = branch_delay,
...
func _update_forks(ray, points, tick, alpha):
	for index in range(ray.forks.size()):
		...
		var reach = (11.0 + abs(_noise(ray.noise_seed + 900, index, tick)) * 19.0) * BOLT_SCALE
		if ray.branch: reach *= 0.72
		var origin = points[point_index]
		var tip_dir = (normal * side + direction * 0.44).normalized()
		var tip_normal = Vector2(-tip_dir.y, tip_dir.x)
		#three segments with their own kink, so the branch bends like the bolt does
		var fork_points = PoolVector2Array()
		fork_points.append(origin)
		for step in range(1, 4):
			var along = reach * float(step) / 3.0
			var wobble = _noise(ray.noise_seed + 1500 + index * 37, step, tick) * reach * 0.22
			if step == 3: wobble *= 0.35
			fork_points.append(origin + tip_dir * along + tip_normal * wobble)
...
func _update_impact(line, center, age, alpha):
	var points = PoolVector2Array()
	var radius = (8.0 + 10.0 * sin(clamp(age / 0.09, 0.0, 1.0) * PI)) * BOLT_SCALE
	var count = 16
	for index in range(count + 1):
