...
func initiate():
	...
	if takers[0].person.get_stat('lust') > 30:
		text += tr("SEXACTION_ASSFINGERING_INITIATE_1")
	else:
		text += tr("SEXACTION_ASSFINGERING_INITIATE_2")
	text += tr("SEXACTION_ASSFINGERING_INITIATE_SUFFIX")
	return text

func reaction(member):
	var text = ''
	if member.energy == 0:
		text = tr("SEXACTION_ASSFINGERING_REACTION_UNCONSCIOUS")
	#elif member.consent == false:
		#TBD
	elif member.sens < 100:
		text = tr("SEXACTION_ASSFINGERING_REACTION_1")
	elif member.sens < 300:
		text = tr("SEXACTION_ASSFINGERING_REACTION_2")
	elif member.sens < 600:
		text = tr("SEXACTION_ASSFINGERING_REACTION_3")
	else:
		text = tr("SEXACTION_ASSFINGERING_REACTION_4")
	return text
