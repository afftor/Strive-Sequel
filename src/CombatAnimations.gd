...
func hp_update(node, args):
	...
	#float damage
	input_handler.FloatText(node, str(args.damage), args.type, 125, args.color, 1, 0.5)#, Vector2(node.get_node('Icon').rect_position.x+25, node.get_node("Icon").rect_position.y+70))
	#update hp bar
