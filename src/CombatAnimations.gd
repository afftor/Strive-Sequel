...
func hp_update(node, args):
	...
	if args.damage_float:
		...
		else: tween.interpolate_callback(input_handler, delay, 'FloatTextArgs', {node = node, text = str(args.damage), type = args.type, size = 125, color = args.color, time = 1, fadetime = 0.5, offset = Vector2(0,0)})
