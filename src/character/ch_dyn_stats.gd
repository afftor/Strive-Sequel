...
func fix_stat_data(stat, data):
	match stat:
		...
		'lustmax':
			...
			if !data.bonuses.has('add'):
				data.bonuses.add = []
			data.bonuses.add.push_back({value = parent.fet_ref().get_stat('thralls_amount') * (7 + 3 * get_stat('sexuals_factor')), src_type = 'other', src_value = 'thralls', timestamp = 0})
		'lusttick':
			if !data.bonuses.has('add'):
				data.bonuses.add = []
			data.bonuses.add.push_back({value = parent.fet_ref().get_stat('thralls_amount') * (0.9 + 0.1 * get_stat('sexuals_factor')), src_type = 'other', src_value = 'thralls', timestamp = 0})
		'trainee_amount':
			if !data.bonuses.has('add'):
				data.bonuses.add = []
