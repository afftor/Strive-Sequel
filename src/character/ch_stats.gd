...
func process_chardata(chardata, unique = false):
	...
	for i in chardata:
		if !(i in ['code', 'slave_class', 'tags','sex_traits', 'sex_skills', 'sex_training', 'personality', 'training_disposition', 'blocked_training_traits', 'training_points', 'traits', 'food_like', 'food_hate', 'classes', 'skills', 'mastery', 'achievement', 'achi_bonus', 'achi_wedding']):
...
func get_pronoun_value(pronoun):
	match pronoun:
		...
		"Succubus": return globals.fastif(statlist.sex == 'male', tr('PRONOUNSUCCUBUS'), tr("PRONOUNSUCCUBUSF")) 
